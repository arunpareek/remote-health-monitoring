...
var gatewayId = process.argv[2];
var icdFile = process.argv[4];

if (!icdFile) {
    icdFile = 'icd.json';
}

if (!dcl.$port.file.exists(icdFile)) {
    dcl.$port.file.append(icdFile, '{}');
}
...
function deviceEnroll(device) {
    ...
    var icdObj = JSON.parse(dcl.$port.file.load(icdFile));
    if (icdObj[gatewayId] && icdObj[gatewayId].length > 1) {
        console.log('------------------TEMPERATURE DEVICE------------------');
        console.log(icdObj[gatewayId][0]);
        console.log('------------------------------------------------------');
        startTemperature(device, icdObj[gatewayId][0]);
        console.log('------------------HUMIDITY DEVICE---------------------');
        console.log(icdObj[gatewayId][1]);
        console.log('------------------------------------------------------');
        startHumidity(device, icdObj[gatewayId][1]);
    } else {
        icdObj[gatewayId] = [];
        dcl.$port.file.store(icdFile, JSON.stringify(icdObj));
        device.registerDevice(dcl.$port.util.uuidv4(), {}, ['urn:com:oracle:iot:device:temperature_sensor'], function (id, error) {
            if (error) {
            ...
            if (id) {
                ...
                console.log('------------------------------------------------------');
                var icdObj = JSON.parse(dcl.$port.file.load(icdFile));
                icdObj[gatewayId].splice(0, 0, id);
                dcl.$port.file.store(icdFile, JSON.stringify(icdObj));
                startTemperature(device, id);
            }
        });

        device.registerDevice(dcl.$port.util.uuidv4(), {}, ['urn:com:oracle:iot:device:humidity_sensor'], function (id, error) {
            if (error) {
                console.log('----------------ERROR ON DEVICE REGISTRATION----------------');
                console.log(error.message);
                console.log('------------------------------------------------------------');
