...
var urlPattern = new RegExp('^(https?:\\/\\/|mqtts:\\/\\/)?'+ // protocol
...
if (options.indexOf('-h') > -1) {
    console.log( "usage:" );
    console.log( " provisioner.sh/bat [option...] <URI> <ID> <Secret> <password>" );
...
function provisionWithParams(){
    switch (typeFlag) {
        case 1:
            dcl.device.TrustedAssetsManager.provision(clientId + '.json', password, host, port, clientId, sharedSecret, trustAnchors, (protocol !== 'https' ? protocol : null));
    ...
    if (!serverUrl) {
        rl.question('Enter Cloud Service URI: ', function (answer) {
            if (urlPattern.test(answer)) {
                serverUrl = answer;
                var matches = urlPattern.exec(serverUrl);
                if ((typeFlag === 1) && matches[1] && (matches[1].indexOf('mqtt') > -1)) {
                    protocol = 'mqtt';
                }
                host = matches[2];
                if (typeof matches[8] !== 'undefined') {
                    port = parseInt(matches[8].substring(1));
                }
                importCerts();
            } else {
